...
def upload_to_article(data):
    ...
    for record_num in range(len(데이타)):
        ...
        Article(id=ids,title=title,content=content,date =date,writer=writer, href=href,title_noun = nouns, 채용=채용,합격자=합격자,의회=의회,관람=관람, 대회=대회, 의견수렴=의견수렴, 업체=업체,  전문교육=전문교육,청소년=청소년,주민자치=주민자치).save()
...
def change_title_key(datas,verbose=1):
    ...
    for i,title in zip(ix,titles):
        print(cc,'번째 글')
        for key, value in change_key(title,verbose).items():
            if key in vocab:
...
def change_content_key(datas,verbose=1):
    ...
    for i,content in zip(ix,contents):
        print(cc,'번째 글')
        for key, value in change_key(content,verbose=verbose).items():
            if key in vocab:
...
def change_key(sentence,verbose=1):
    t= time.time()
