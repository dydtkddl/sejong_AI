...
def search_2(request):
    ...
    if page != str(cases//10+1):
        ...
        after_page = page
    print(page,'페이지 넘어가는데',time.time()-time_,'초 소요')
    return render(request, 'app/page.html', context = {'search_period':search_period,'key':ory_key,'cases':cases, 'page_obj':page_obj, 'paginator':paginator,'before_page_num':before_page,'page_num':page,'after_page_num':after_page})
def up(request):
    mode = request.GET.get('mode')
    ...
    data = pd.DataFrame({'id':ids,'title':title,'content':contents})
    if mode=='title':
        change_title_key(data,verbose=0)
    elif mode == 'content':
        change_content_key(data ,verbose=0)
